...
for file, targets in files.items():
    ...
    content = content.replace(
        "model = Conv1DDuelingDQN(n_actions)", "model = DuelingDQNNetwork(action_dim=n_actions)"
    )
    ...
    if "training.py" in file:
        ...
        content = content.replace(
            "model = DuelingDQNNetwork(n_actions)",
            "model = DuelingDQNNetwork(action_dim=n_actions)",
        )
